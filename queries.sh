...
echo "$($PSQL "SELECT w.name FROM games INNER JOIN teams as w ON games.winner_id=w.team_id WHERE year=2014 AND round='Eighth-Final' UNION SELECT o.name FROM games INNER JOIN teams as o ON games.opponent_id=o.team_id WHERE year=2014 AND round='Eighth-Final'")"
